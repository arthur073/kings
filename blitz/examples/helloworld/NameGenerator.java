...
public class NameGenerator {
	...
	private List vocals = (List) new ArrayList();
	private List startConsonants = (List) new ArrayList();
	private List endConsonants = (List) new ArrayList();
	private List nameInstructions = (List) new ArrayList();
	...
	public NameGenerator() {
		...
		((ArrayList) this.vocals).addAll(Arrays.asList(demoVocals));
		((ArrayList) this.startConsonants).addAll(Arrays.asList(demoStartConsonants));
		((ArrayList) this.endConsonants).addAll(Arrays.asList(demoEndConsonants));
		((ArrayList) this.nameInstructions).addAll(Arrays.asList(nameInstructions));
	}
	...
	public NameGenerator(String[] vocals, String[] startConsonants,
		...
		((ArrayList) this.vocals).addAll(Arrays.asList(vocals));
		((ArrayList) this.startConsonants).addAll(Arrays.asList(startConsonants));
		((ArrayList) this.endConsonants).addAll(Arrays.asList(endConsonants));
	}
	...
	private String getNameByInstructions(String nameInstructions) {
		...
		for (int i = 0; i < l; i++) { char x = nameInstructions.charAt(0); switch (x) { case 'v': name += getRandomElementFrom(vocals); break; case 'c': name += getRandomElementFrom(startConsonants); break; case 'd': name += getRandomElementFrom(endConsonants); break; } nameInstructions = nameInstructions.substring(1); } return name; } private String firstCharUppercase(String name) { return Character.toString(name.charAt(0)).toUpperCase() + name.substring(1); } private String getRandomElementFrom(List v) {
			return (String) ((ArrayList) v).get(randomInt(0, ((ArrayList) v).size() - 1));
		}
}
